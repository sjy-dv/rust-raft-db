...
impl SjySQL {
    /// Creates a new sjySQL REPL for the given server host and port
    async fn new(host: &str, port: u16) -> Result<Self> {
        Ok(Self {
            client: Client::new((host, port)).await?,
            editor: Editor::new()?,
            history_path: std::env::var_os("HOME")
                .map(|home| std::path::Path::new(&home).join(".sjysql.history")),
            show_headers: false,
        })
    }
...
"#,
    ...
    fn prompt(&mut self) -> Result<Option<String>> {
        let prompt = match self.client.txn() {
            Some((version, false)) => format!("rrdb:{}> ", version),
            Some((version, true)) => format!("rrdb@{}> ", version),
            None => "rrdb> ".into(),
        };
    ...
    async fn run(&mut self) -> Result<()> {
        ...
        let status = self.client.status().await?;
        println!(
            "Connected to rrdb node \"{}\". Enter !help for instructions.",
            status.raft.server
        );
